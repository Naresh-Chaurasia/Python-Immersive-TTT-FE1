...
logger.info("FastAPI application created: title='%s'", app.title)
...
async def _ingest_job():
    ...
    """
    logger.info("Background ingestion job started")
    ...
    logger.info("Ingestion status set to 'running'")
...
async def kick_off_ingest():
    ...
    async with _ingest_lock:
        ...
        if _ingest_task and not _ingest_task.done():
            logger.warning("Ingestion already running — returning 409")
            return JSONResponse(
...
async def ingest_status():
    ...
    """
    logger.debug("GET /ingest/status → %s", _ingest_last["status"])
    return {
        "ok": True,
        **_ingest_last
    }
...
async def ask(q: Ask):
    ...
    """
    logger.info("POST /ask called: question='%s'", q.question)
    ...
    category = None  # Default to no category filter
    logger.info("Using category filter: %s", category)
    ...
    logger.info("/ask execution took %.2f seconds", elapsed)
