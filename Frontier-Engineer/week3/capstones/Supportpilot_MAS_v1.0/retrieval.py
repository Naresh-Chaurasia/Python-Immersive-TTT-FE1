...
class KnowledgeRetriever:
    ...
    def __init__(self, kb_dir: Path = KB_DIR):
        ...
        logger.info("Knowledge base index built: %d chunk(s) from %s", len(self.chunks), kb_dir)
    ...
    def retrieve(self, query: str, top_k: int = 3, exclude_internal: bool = True) -> list[dict]:
        ...
        for idx in ranked_idx:
            ...
            if len(results) >= top_k:
                break
        logger.debug("Retrieved chunks for '%s': %s", query, [r["source_doc"] for r in results])
        return results
...
if __name__ == "__main__":
    ...
    logger.info("Loaded %d chunks from %s", len(retriever.chunks), KB_DIR)
    for r in retriever.retrieve("customer wants a refund for a damaged item"):
        logger.info("  [%s] %s — %s", r["relevance_score"], r["source_doc"], r["section"])
