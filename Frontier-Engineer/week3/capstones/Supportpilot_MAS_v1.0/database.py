...
# Schema for the three tables: customers, their orders, and support tickets
SCHEMA = """
CREATE TABLE IF NOT EXISTS customers (
    customer_id TEXT PRIMARY KEY,
    name TEXT,
    email TEXT,
    account_standing TEXT,   -- "good", "vip", or "flagged" (fraud-risk)
    signup_date TEXT
);
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    item TEXT,
    amount REAL,
    status TEXT,             -- "delivered", "in_transit", "returned", "cancelled"
    order_date TEXT,
...
# Seed customers: (customer_id, name, email, account_standing, signup_date)
CUSTOMERS = [
    ...
    # Edge case: brand new customer, deliberately given no orders below
    ("CUST011", "Meera Pillai", "meera.pillai@example.com", "good", "2025-06-01"),
]

# Item names cycled through when generating synthetic orders
_ITEMS = [
...
# Generate synthetic orders for every customer except CUST011 (kept order-less)
ORDERS = []
_order_counter = 1  # gives each order a unique, incrementing ID
for cust_id in [c[0] for c in CUSTOMERS if c[0] != "CUST011"]:
    # CUST002 gets a single high-value order to exercise the refund
    # hard-escalation rule; everyone else gets 5 regular orders.
    n_orders = 5 if cust_id != "CUST002" else 1
    for i in range(n_orders):
        oid = f"ORD{_order_counter:04d}"
        item = _ITEMS[_order_counter % len(_ITEMS)]
        # Deterministic "pseudo-random" amount so seed data is reproducible;
        # CUST002's order is fixed high to trigger the escalation rule.
...
def init_db(force: bool = False) -> None:
    """Create the schema and seed customers/orders. If force=True, wipe any existing DB file first."""
    if force and DB_PATH.exists():
        DB_PATH.unlink()
        logger.info("Removed existing database file at %s", DB_PATH)
    conn = sqlite3.connect(DB_PATH)
    conn.executescript(SCHEMA)
    # "INSERT OR IGNORE" skips rows that already exist, so seeding is safe to repeat
    conn.executemany(
    ...
    conn.commit()
    conn.close()
    logger.info("Database ready: %d customers, %d orders", len(CUSTOMERS), len(ORDERS))


def get_customer(customer_id: str) -> dict | None:
    """Fetch a single customer row as a dict, or None if not found."""
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row  # lets rows be converted to dicts below
    row = conn.execute(
        "SELECT * FROM customers WHERE customer_id = ?", (customer_id,)
    ).fetchone()
    conn.close()
    logger.debug("Customer lookup %s: %s", customer_id, "found" if row else "not found")
    return dict(row) if row else None


def get_orders(customer_id: str) -> list[dict]:
    """Fetch all orders for a customer, most recent order_date first."""
    conn = sqlite3.connect(DB_PATH)
    ...
    conn.close()
    logger.debug("Orders lookup %s: %d order(s)", customer_id, len(rows))
    return [dict(r) for r in rows]


def get_prior_ticket_count(customer_id: str) -> int:
    """Count how many support tickets already exist for this customer."""
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(
...
# Running this module directly (re)seeds the database from scratch
if __name__ == "__main__":
    init_db(force=True)
    logger.info("Seeded database at %s", DB_PATH)
    logger.info("Customers: %d | Orders: %d", len(CUSTOMERS), len(ORDERS))
