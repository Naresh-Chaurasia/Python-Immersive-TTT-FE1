...
def run_ticket(ticket_id: str, ticket_text: str, customer_id: str | None = None) -> dict:
    ...
    logger.info("--- Ticket %s done: %s ---", ticket_id, resolution_path.upper())
    return report


def pretty_print_report(report: dict) -> None:
    logger.info("\n%s", "=" * 70)
    logger.info("Ticket: %s  |  Path: %s", report["ticket_id"], report["resolution_path"].upper())
    logger.info("%s", "=" * 70)
    c = report["classification"]
    # Use f-string interpolation (not %s) for these fields: when pydantic validation
    # is active, they're Enum members whose __str__ gives "IssueType.ORDER_STATUS"
    # rather than the plain value — f-strings call __format__, which resolves correctly.
    logger.info(
        "Issue type: %s | Urgency: %s | Sentiment: %s | Confidence: %.2f",
        f"{c['issue_type']}", f"{c['urgency']}", f"{c['sentiment']}", c["confidence"],
    )
    logger.info("KB chunks used: %s", report["kb_chunks_used"] or "none")
    if report["resolution_path"] == "auto_resolved":
        logger.info("Response: %s", report["final_response"])
    else:
        esc = report["escalation"]
        logger.info("ESCALATED — type: %s | reason: %s", esc["escalation_type"], esc["reason"])
    logger.info("Agent trace: %s", " -> ".join(report["agent_trace"]))
