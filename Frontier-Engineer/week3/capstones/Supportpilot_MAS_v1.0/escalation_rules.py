...
def decide_escalation(issue_type: str, sentiment: str, urgency: str, confidence: float,
    ...
    if hard:
        logger.info("Escalation decision: ESCALATE (hard_category) — %s", hard_reason)
    ...
    if soft:
        ...
        logger.info("Escalation decision: ESCALATE (%s) — %s", etype, soft_reason)
        return {"should_escalate": True, "reason": soft_reason, "escalation_type": etype}

    logger.info("Escalation decision: AUTO-RESOLVE — passed all checks")
...
if __name__ == "__main__":
    ...
    for c in cases:
        logger.info(decide_escalation(**c))
