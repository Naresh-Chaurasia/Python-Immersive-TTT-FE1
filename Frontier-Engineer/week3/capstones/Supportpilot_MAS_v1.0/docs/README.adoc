...
| Keyword/regex heuristics standing in for the LLM. Zero API calls, zero cost, fully offline.

| `live`
| `export SUPPORTPILOT_MODE=live` +
`export ANTHROPIC_API_KEY=sk-...`
| Real calls to the Anthropic API (`claude-sonnet-5` by default — override with `SUPPORTPILOT_MODEL`) for classification, drafting, and validation.
